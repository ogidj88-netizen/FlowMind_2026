...
class CanonicalDispatcher:
    ...
    def _build_transition_candidate(
        ...
        *,
        ...
        self._assert_phase_guards(current_state, target_phase)
    ...
    def _assert_phase_guards(self, current_state: Mapping[str, Any], target_phase: str) -> None:
        current_phase = current_state["phase"]

        if current_phase == "ASSEMBLY" and target_phase == "QA":
        ...
        if current_phase == "QA" and target_phase == "READY_FOR_UPLOAD":
            if current_state.get("qa_passed") is not True:
                raise DispatcherTransitionError(
                    "cannot transition QA -> READY_FOR_UPLOAD while qa_passed is not true"
                )

        if current_phase == "READY_FOR_UPLOAD" and target_phase == "UPLOADED":
            if current_state.get("approved_for_upload") is not True:
                raise DispatcherTransitionError(
